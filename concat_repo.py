...
Includes only the files required for reproducibility and verification:
- `article_blackhole_inevitable_en.md`
- `LeanBh/*.lean`
- `get_phi_years.py` and helper scripts
- `logic-graph.yml`
- `plugins/**/*.py`
- `README.md`
- `setup.py`, `lakefile.toml`, `lake-manifest.json`

Everything else (egg-info, .lake, .git, build, dist, node_modules, caches, images, etc.) is ignored.
...
def write_concat() -> None:
    """Create a single text snapshot of all included resources."""
